const TableRow = ({ record, handleClickDelete, handleClickUpdate }) => {
  ...
  return (
    <tr key={record.id}>
      <td className="border  py-1 px-2 bg-blue-50">{record.category}</td>
      <td className="border py-1  px-2 bg-white">{record.item}</td>
      <td className="border  py-1 px-2 bg-white">{record.price} €</td>
      <td className="border  py-1 px-2 bg-white">{record.date}</td>
