...
const Table = ({ records, handleClickUpdate, handleDeleteAndUpdate }) => {
  ...
  const sortRecords = (records, criteria, order) => {
    ...
    return records.length === 0 ? (
      <p className="text-2xl text-yellow-600">Sorry, no records yet.</p>
    ) : (
      <>
        <p className="mb-4 flex flex-col text-center text-sm font-bold text-green-800 border-solid py-2 border border-green-600 rounded-md">
          Summary spent {summary.totalPrice} € on {summary.numItems} items
        </p>
